...
test("homepage renders both catalog tabs and default free-tools", async ({ page }) => {
  await page.goto("/");
  ...
  await test.expect(tabs.nth(1)).toHaveText("Free tools");

  // free-tools is the default catalog
  await test.expect(page.locator("[data-catalog-panels=free-tools]")).toBeVisible();
...
test("switching catalog swaps panels and updates the URL", async ({ page }) => {
  ...
  await test.expect(page.locator("[data-catalog-panels=free-tools]")).toBeHidden();
...
test("search filters entries by name", async ({ page }) => {
  ...
  // Neon lives in the free-tools catalog (default active)
  const neonRow = page.locator('[data-entry]').filter({ hasText: "Neon" }).first();
  await test.expect(neonRow).toBeVisible();
  await test.expect(page.locator("[data-catalog-panels=free-tools]")).toContainText("Neon");
});
